...
# do to clean it up? Since there were multiple rows for each class, we had to 
# group by the class criteria and then summarize by the total undergraduate 
# count
```
...
sep_new <- 
  ...
  rename(id = course_id,
         ...
         u_grad_new = u_grad) %>%
  group_by(id, title, name, department) %>%
  summarize(u_grad_new = sum(u_grad_new), .groups = "drop") %>%
...
```{r}
# We need to determine what actual data is necessary to compare so we remove
# things outside the scope / duplicates. We need both the data sets to have the
# same qualifying filters (ie undergrads) to make comparisons between them.
```

After we have a model, Temperance suggests we should:

```{r}
# Eliminating small classes might create error since we are filtering for 10
# people and a class with >10 people might not be included after a drop
# deadline.
# Uncertainty since we don't know if people left because they took a leave or just changed classes. 
# internal and external validity -- predicting past does not mean you can predict the future b/c overfitting
...
sep <- inner_join(sep_old, sep_new, by = c("id", "title", "name", "department")) %>%
  mutate(difference = u_grad_new - u_grad) %>%
  arrange(desc(difference)) %>%
  slice(1:5, 412:416) %>%
  ggplot(aes(x = title, y = difference, fill = department)) +
  geom_col() +
